...
/**
	...
	License: GPL2
		...
		Copyright 2010-2018  Tim Berneman  (email: tberneman@gmail.com)
...
//if (!defined('NOAA_WEATHER_WIDGET_VERSION'))
define('NOAA_WEATHER_WIDGET_VERSION', '3.4.1');
...
register_uninstall_hook( __FILE__, 'uninstall_NOAA_Weather_widget' );
...
function deactivate_NOAA_Weather_widget() {
	// Remove cron entry that downloads the weather file to save on system resources
	wp_clear_scheduled_hook( 'Get_NOAA_Weather' );
}

function uninstall_NOAA_Weather_widget() {
...
function init_NOAA_Weather_widget() {
	...
	wp_enqueue_style( 'NOAA_Weather_Widget_Stylesheet' , plugin_dir_url(__FILE__) . '/noaa-weather.css' );
}
 ...
 */
function Get_NOAA_Weather_File_With_HTTP( $acode ) {
	// Get current conditions
	$result = wp_remote_get( "http://w1.weather.gov/xml/current_obs/{$acode}.xml" );
	if( is_array($result) ) {
		$fp = @fopen(dirname( __FILE__) . "/weather-current-{$acode}.xml", "w+" );	if ( $fp !== false ) {
...
class NOAA_Weather_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $noaa_code ) {
			// Check to see if XML file exists and if it doesn't then call the function to get it
			$xmlexist = @simplexml_load_file(dirname(__FILE__) . "/weather-current-".$noaa_code.".xml");
			if ( $xmlexist === false ) {
				Get_NOAA_Weather_File_With_HTTP( $noaa_code );
			}
			// If the XML file doesn't exist at this point after the previous check then we have big problems!
	...
	function form( $instance ) {
		...
		<p>
			OPTIONAL: Use this field to override the location (city, airport, etc.) that is provided in the downloaded NOAA weather file.
		</p>
